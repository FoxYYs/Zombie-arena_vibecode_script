...
function Library:CreateWindow(config)
	...
	function Window:AddTab(name, icon)
		...
		-- Dropdown (overlay в корневом gui, ZIndex 100, плашка всегда 44px, авто-скролл)
		function Tab:AddDropdown(opts)
			...
			-- список вынесен в корневой gui (ScreenGui), чтобы ClipsDescendants контейнеров его не резал
			local list = Util.Create("Frame", {
				Parent = gui, Size = UDim2.new(0, 150, 0, 0), BackgroundColor3 = Theme.Element,
				BackgroundTransparency = 0.02, Visible = false, ClipsDescendants = true, ZIndex = 100,
			})
			Util.Corner(list, 6)
			...
			-- соединения объявлены заранее, чтобы их видели замыкания в rebuild()
			local scrollConn, posConn, visConn
			...
			local function syncPos()
				-- позиция относительно корневого gui, т.к. list теперь его потомок
				local hp, gp = header.AbsolutePosition, gui.AbsolutePosition
				list.Position = UDim2.new(0, hp.X - gp.X, 0, (hp.Y - gp.Y) + header.AbsoluteSize.Y + 4)
			end
			...
			local function rebuild()
				...
				for _, opt in ipairs(options) do
					local row = Util.Create("TextButton", {
						...
						BackgroundTransparency = 0.2, Text = "", AutoButtonColor = false, ZIndex = 101,
					})
					Util.Corner(row, 5)
					Util.Create("TextLabel", {
					...
					row.MouseButton1Click:Connect(function()
						...
						Util.Tween(list, 0.18, { Size = UDim2.new(0, header.AbsoluteSize.X, 0, 0) })
			...
			header.MouseButton1Click:Connect(function()
				...
				if open then
					...
					list.Visible = true
					list.Size = UDim2.new(0, header.AbsoluteSize.X, 0, 0)
					syncPos()
					ensureVisible(listH)
					syncPos()
					Util.Tween(arrow, 0.2, { Rotation = 180 })
					Util.Tween(list, 0.2, { Size = UDim2.new(0, header.AbsoluteSize.X, 0, listH) })
					scrollConn = container:GetPropertyChangedSignal("CanvasPosition"):Connect(syncPos)
					-- фикс перетаскивания окна: следим за позицией хедера
					posConn = header:GetPropertyChangedSignal("AbsolutePosition"):Connect(syncPos)
					-- фикс смены вкладок: при скрытии страницы принудительно закрываем дропдаун
					visConn = page:GetPropertyChangedSignal("Visible"):Connect(function()
						if not page.Visible then
							open = false
							disconnectAll()
							Util.Tween(arrow, 0.2, { Rotation = 0 })
							Util.Tween(list, 0.2, { Size = UDim2.new(0, header.AbsoluteSize.X, 0, 0) })
							list.Visible = false
						end
					end)
				else
			...
			-- защита от утечек: при уничтожении окна гасим соединения и удаляем оверлей-список
